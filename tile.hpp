...
// Minimal per‑voxel physics state – only what changes every frame for every tile.
// Soil chemistry, snow cover, structural integrity, etc. are stored sparsely
// in Game_map to save memory.
struct TileState {
...
struct Tile {
    ...
    // Query helpers
    bool is_wall()  const noexcept { auto& p = mat(); return p.is_solid && p.is_opaque; }
    bool is_floor() const noexcept { auto& p = mat(); return !p.is_solid && !p.is_opaque; }
