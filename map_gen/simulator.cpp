...
void MapSimulator::initialize(Game_map& game_map, int seed) {
    ...
    soil_variation_ = hydro::compute_soil_variation(ground_z_, width_, height_, noise_, params_);
...
void MapSimulator::simulate_substep(Game_map& game_map, int substep, int year) {
    float season_phase = (float)substep / (float)params_.substeps_per_year; // 0..1 over the year
    ...
    std::vector<float> precip = climate_.step_precipitation(ground_z_, noise_, day_index);

    auto end = std::chrono::high_resolution_clock::now();
    step_timings_["Atmosphere"] += std::chrono::duration<double>(end - start).count();

    // --- 2. Infiltration ---
    start = std::chrono::high_resolution_clock::now();
    hydro::apply_precipitation(game_map, ground_z_, precip, params_);
    end = std::chrono::high_resolution_clock::now();
    step_timings_["Infiltration"] += std::chrono::duration<double>(end - start).count();

    // --- 3. Soil column: percolation (bucket model) + capillary rise ---
    start = std::chrono::high_resolution_clock::now();
    hydro::soil_percolation_step(game_map, ground_z_, soil_variation_, params_, groundwater_);
    ...
    step_timings_["Surface"] += std::chrono::duration<double>(end - start).count();
    ...
    step_timings_["Evapotranspiration"] += std::chrono::duration<double>(end - start).count();

    // --- 6. Groundwater: slow diffusion + baseflow to surface ---
    start = std::chrono::high_resolution_clock::now();
    if (substep % params_.groundwater_update_interval == 0) {
        std::vector<float> discharge = groundwater_.update(ground_z_, game_map);
        for (int y = 0; y < height_; ++y) {
            for (int x = 0; x < width_; ++x) {
                int i = y * width_ + x;
                if (discharge[i] > 0.0f) {
                    hydro::add_surface_water(game_map, x, y, ground_z_[i], discharge[i], params_);
                }
            }
        }
    }
    end = std::chrono::high_resolution_clock::now();
    step_timings_["Groundwater"] += std::chrono::duration<double>(end - start).count();
}
