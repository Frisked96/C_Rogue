...
void MapSimulator::run(Game_map &game_map, int seed) {
  ...
  for (int year = 0; year < num_years; ++year) {
    ...
    atmosphere_water =
        std::min(atmosphere_water, (float)(width * height) * 2.0f);
  }
}

void MapSimulator::apply_raindrops(Game_map &game_map, float total_water) {
  int width = game_map.get_width(), height = game_map.get_height(),
      depth = game_map.get_depth();
...
float MapSimulator::simulate_hydrology(Game_map &game_map) {
  ...
  // Optimization: find max terrain height to skip empty sky (hardcoded limit
  // for now based on generation)
      ...
      for (int x = 0; x < width; ++x) {
        ...
        if (x == 0 || x == width - 1 || y == 0 || y == height - 1) {
          if (t.state.moisture > 0.001f) {
            float runoff =
                t.state.moisture * 0.5f; // 50% drain per step at edges
            t.state.moisture -= runoff;
          }
        }

        // Optimization: Skip empty air and dry tiles
        if (t.state.moisture <= 0.0001f && t.material == MaterialType::AIR)
          continue;

        // --- 2. Thermodynamic Evaporation ---
        bool is_surface =
            (z == depth - 1) ||
            (game_map.get_tile(x, y, z + 1).material == MaterialType::AIR);
        if (is_surface && t.state.moisture > 0) {
          float temp_factor =
              std::max(0.1f, (t.state.temperature - 273.15f) / 15.0f);
        ...
        if (z > 0) {
          ...
          float capacity = (below.material == MaterialType::AIR ||
                            below.material == MaterialType::WATER_FRESH)
                               ? 1.0f
                               : below.effective_porosity();

          float k = t.mat().permeability;
          float flow = std::min(t.state.moisture,
                                (capacity - below.state.moisture) * k * 100.0f);

          if (flow > 0.0001f) {
            t.state.moisture -= flow;
            below.state.moisture += flow;
            if (below.material == MaterialType::AIR &&
                below.state.moisture > 0.05f) {
        ...
        float flow_threshold = (t.material == MaterialType::WATER_FRESH)
                                   ? 0.01f
                                   : (t.effective_porosity() * 0.2f);
        if (t.state.moisture > flow_threshold) {
          int dx[] = {1, -1, 0, 0}, dy[] = {0, 0, 1, -1};
          for (int i = 0; i < 4; ++i) {
            int nx = x + dx[i], ny = y + dy[i];
            if (!game_map.is_in_bounds(nx, ny, z))
              continue;

            Tile &nt = const_cast<Tile &>(game_map.get_tile(nx, ny, z));
            float nt_cap = (nt.material == MaterialType::WATER_FRESH ||
                            nt.material == MaterialType::AIR)
                               ? 1.0f
                               : nt.effective_porosity();

            float k_avg = (t.mat().permeability + nt.mat().permeability) * 0.5f;
            float flow_mult = (t.material == MaterialType::WATER_FRESH ||
                               nt.material == MaterialType::WATER_FRESH)
                                  ? 0.2f
                                  : (k_avg * 50.0f);

            if (z > 0 && game_map.get_tile(nx, ny, z - 1).material ==
                             MaterialType::AIR) {
              flow_mult *= 1.5f;
            }
        ...
        float max_vol = (t.material == MaterialType::WATER_FRESH)
                            ? 1.0f
                            : t.effective_porosity();
...
// Dummy implementation since balance_basins is removed from the loop but still
// in header
