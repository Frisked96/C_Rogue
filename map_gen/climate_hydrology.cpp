...
// Avoids repeated OS calls and makes the value consistent across functions.
...
void ClimateSystem::update_wind(const std::vector<int> &ground_z,
  ...
  for (int t = 0; t < num_threads; ++t) {
    ...
    // Each thread gets its own NoiseGen copy (same seed/state, no sharing).
        ...
        std::launch::async, [this, start_y, end_y, base_u, base_v, elev, local_noise = noise]() mutable {
          for (int y = start_y; y < end_y; ++y) {
            for (int x = 0; x < w_; ++x) {
              ...
              if (speed < 1e-5f) {
                // Truly calm cell — write zeros and move on rather than
                // forcing an arbitrary eastward direction.
                ClimateCell &c = cells_[idx(x, y)];
                c.wind_u = 0.0f;
                c.wind_v = 0.0f;
                c.upslope = 0.0f;
...
void ClimateSystem::advect() {
  ...
  for (int y = 0; y < h_; ++y) {
    for (int x = 0; x < w_; ++x) {
      ...
      if (nx >= 0 && nx < w_)
        delta[idx(nx, y)] += flux_x;
      if (ny >= 0 && ny < h_)
        delta[idx(x, ny)] += flux_y;
    }
  }
...
std::vector<float> GroundwaterGrid::update(const std::vector<int> &ground_z,
                                           const Game_map &map) {
...
void soil_percolation_step(Game_map &map, const std::vector<int> &ground_z,
  ...
  for (int t = 0; t < num_threads; ++t) {
    ...
    futures.push_back(std::async(std::launch::async, [=, &map, &ground_z,
                                                      &soil_variation, &p,
                                                      &groundwater]() {
      for (int y = start_y; y < end_y; ++y) {
        for (int x = 0; x < width; ++x) {
    ...
    for (int x = 0; x < width; ++x) {
      ...
      if (best_diff > 0.0f) {
        ...
        if (flow > 1e-6f) {
          outflow[i] += flow;
          if (best_offmap)
            runoff_to_ocean += flow;
          else
            inflow[best_idx] += flow;
        }
      }
    }
  }
...
float evapotranspiration_step(Game_map &map, ClimateSystem &climate,
  ...
  for (int t = 0; t < num_threads; ++t) {
    ...
    futures.push_back(
        std::async(std::launch::async, [=, &map, &climate, &ground_z, &p]() {
