...
//
// -----------------------------------------------------------------------
// ASSUMED Game_map INTERFACE (matches the original simulator.cpp exactly,
// nothing new is required of Game_map itself):
//
//   int  get_width()  const;
...
//
// All new state (climate grids, water table, heightmap, noise) is owned by
// MapSimulator (see simulator.hpp), not by Game_map.
//
// -----------------------------------------------------------------------
// UNITS / CONVENTIONS
//
...
public:
    ...
    float noise2D(float x, float y) {
        return fnl_single_.GetNoise(x, y);
    }
    ...
    float fbm2D(float x, float y,
                ...
                float gain = 0.5f) {
        if (octaves != last_octaves_ || lacunarity != last_lacunarity_ || gain != last_gain_) {
...
struct Params {
    ...
    float lapse_rate_K_per_tile = 0.45f;  // "gamified" lapse rate (K lost per metre of elevation)
    float orographic_gain      = 4.0f;    // extra cooling per unit of upslope wind component
    float qsat_ref   = 0.05f;  // saturation vapor "capacity" at sea_level_temp_K
    float qsat_k     = 0.07f;  // exponential rate, per Kelvin (Clausius-Clapeyron-like)
    float rain_out_fraction = 0.35f; // fraction of supersaturation that rains out per substep
    float ocean_humidity    = 0.05f; // boundary inflow humidity (windward edges)
    float boundary_relax    = 0.25f; // how fast windward edge cells relax toward ocean_humidity
    float convective_threshold = 0.6f;  // noise threshold for spontaneous storms
    float convective_intensity = 0.04f; // extra rain depth at full storm intensity
    float wind_advect_scale = 0.4f; // converts wind "speed" units into fraction-of-vapor-moved
    int   wind_update_interval = 10; // substeps between wind-field refreshes

    // --- Soil hydrology (bucket model) ---
    float percolation_rate_scale = 1.2f; // multiplies permeability-derived K(theta)
    float capillary_rate = 0.05f;        // fraction of vertical moisture gradient exchanged/substep
    float soil_variation_amplitude = 0.2f; // +/- fractional variation in porosity/capacity per column

    // --- Surface water (D8 overland flow) ---
    float overland_flow_fraction = 0.35f; // fraction of head-difference moved per substep
    float pond_form_threshold = 0.02f;    // moisture above which AIR becomes WATER_FRESH

    // --- Groundwater ---
    float groundwater_porosity = 0.2f;       // converts recharge volume <-> water-table height
    float groundwater_diffusion_rate = 0.08f; // fraction of neighbour-difference smoothed per update
    int   groundwater_update_interval = 10;   // substeps between groundwater updates

    // --- Evaporation / evapotranspiration ---
    float evap_coeff = 0.08f; // overall rate multiplier for VPD-driven evaporation
};
...
public:
    ...
    void update_wind(const std::vector<int>& ground_z, NoiseGen& noise, float season_phase);
    ...
    float total_vapor() const {
        float s = 0.0f;
        for (const auto& c : cells_) s += c.vapor;
        return s;
    }
...
public:
    ...
    std::vector<float> update(const std::vector<int>& ground_z, const Game_map& map);
...
// (i.e. the tile at ground_z+1), converting AIR -> WATER_FRESH as needed and
