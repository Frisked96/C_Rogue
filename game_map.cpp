...
Game_map::Game_map(int w, int h, int d)
    : width(w), height(h), depth(d), map(w * h * d, Tiles::Wall),
...
bool Game_map::can_plant(int x, int y, int z) const {
  ...
  if (tile.state.temperature <= 278.0f)
    return false;
  if (tile.state.liquid_volume <= tile.wilting_point())
    return false;
  ...
  float avg_nutrient =
      (chem.nutrients_N + chem.nutrients_P + chem.nutrients_K) / 3.0f;
  if (avg_nutrient < 0.05f || chem.pH < 4.0f || chem.pH > 9.0f)
    return false;
  if (chem.contamination > 0.8f)
    return false;
...
void Game_map::generate(int seed, ObjectPrototypeDB *proto_db,
