...
struct MaterialProperties {
  ...
  float fertility_base; // 0–1, innate nutrient richness (only meaningful for
                        // soils)
...
struct TileState {
  ...
  float ice_fraction; // 0–1, proportion of pore water that is frozen; 0 unless
                      // temperature<273K
  ...
  float nutrients_P;
  float nutrients_K;
  float organic_matter; // 0–1 fraction, affects water retention and nutrient
                        // regeneration
  float pH;             // 0–14

  // --- mechanical ---
  float compaction; // 0 = loose, 1 = fully compacted (reduces porosity,
                    // infiltration, root penetration)
  float structural_integrity; // 1 = intact, 0 = collapsed (used for
                              // ceilings/walls)

  // --- surface cover ---
  float snow_depth;  // metres (0–1, since tile is 1 m tall)
  float litter_mass; // kg of leaf litter / debris (fuels fire, decays into
                     // organic matter)
