...
void populate(Game_map &map, ObjectPrototypeDB &proto_db,
  ...
  for (uint16_t proto_id = 1; proto_id < (uint16_t)proto_db.size();
       ++proto_id) {
    ...
    for (int y = 0; y < height; ++y) {
      for (int x = 0; x < width; ++x) {
        ...
        if (req.requires_ground) {
          ...
          if (ground.material != req.ground_material)
            continue;

          // Soil moisture
          float moisture = ground.state.liquid_volume;
          if (moisture < req.min_soil_moisture)
            continue;
          if (moisture > req.max_soil_moisture)
            continue;
          ...
          if (proto.behavior == ObjectBehavior::VEGETATION) {
            ...
            float normalized = (range > 0.001f)
                                   ? (moisture - req.min_soil_moisture) / range
                                   : 0.0f;

            // Scale probability: baseline at min moisture, up to 4x higher near
            // ideal moisture (30-50% normalized)
