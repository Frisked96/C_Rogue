...
void ObjectManager::kill(ObjectUID uid, Game_map *map) {
  ...
  if (slot.obj.vegetation && map) {
    if (map->is_in_bounds(slot.obj.x, slot.obj.y, slot.obj.z)) {
      map->get_surface(slot.obj.x, slot.obj.y).flow_blockage -=
          slot.obj.vegetation->current_flow_blockage;
      map->get_surface(slot.obj.x, slot.obj.y).flow_blockage = std::max(
          0.0f, map->get_surface(slot.obj.x, slot.obj.y).flow_blockage);
    }
  }
...
void ObjectManager::tick(Game_map *map, bool is_world_gen) {
  ...
  for (auto &slot : slots_) {
    ...
    if (do_medium && proto.tick_freq <= TickFrequency::MEDIUM &&
        proto.is_living) {
    ...
    if (do_low) {
      ...
      if (proto.behavior == ObjectBehavior::VEGETATION && obj.vegetation &&
          map) {
...
void ObjectManager::tick_high(ObjectInstance &obj,
  ...
  if (obj.life) {
    ...
    if (obj.life->hunger > 0.8f && obj.life->current_goal == GoalType::IDLE) {
      ...
      event_bus_.announce(EventType::LIFE_STATE_CHANGED,
                          {obj.uid, 0, obj.life->hunger});
    }
    ...
    if (obj.life->fatigue > 0.9f && obj.life->current_goal != GoalType::FLEE) {
      ...
      event_bus_.announce(EventType::LIFE_STATE_CHANGED,
                          {obj.uid, 0, obj.life->fatigue});
    }
  }
}

void ObjectManager::tick_medium(ObjectInstance &obj,
                                const ObjectPrototype & /*proto*/) {
  ...
  obj.life->hunger =
      std::min(1.0f, obj.life->hunger + obj.life->hunger_rate * 10.0f);
  obj.life->fatigue =
      std::min(1.0f, obj.life->fatigue + obj.life->fatigue_rate * 10.0f);
  obj.life->morale =
      std::max(0.0f, obj.life->morale - obj.life->morale_decay * 10.0f);
...
void ObjectManager::tick_vegetation(ObjectInstance &obj,
                                    const ObjectPrototype & /*proto*/,
                                    Game_map *map, bool is_world_gen) {
  if (!obj.vegetation || !map)
    return;

  if (is_world_gen) {
    obj.vegetation->age++;
    obj.vegetation->canopy = std::min(2.0f, 1.0f + obj.vegetation->age * 0.02f);
    obj.vegetation->canopy_density =
        std::min(1.0f, obj.vegetation->age * 0.01f);
  }
  ...
  if (map->is_in_bounds(obj.x, obj.y, obj.z)) {
    ...
    if (is_world_gen) {
      if (water_level > 0.02f) {
        ...
        obj.vegetation->water_damage =
            std::max(0.0f, obj.vegetation->water_damage - 0.05f);
      }

      float max_water_damage = 1.0f + obj.vegetation->canopy_density * 9.0f;
      if (obj.vegetation->water_damage > max_water_damage ||
          water_level > 1.5f) {
        obj.health = 0.0f;
        event_bus_.announce(EventType::HEALTH_CHANGED,
                            {obj.uid, 0, obj.health});
        death_queue_.push_back(obj.uid);
        return;
      }
    }
  ...
  if (is_world_gen) {
    ...
    if (obj.vegetation->age % 5 == 0) {
      ...
      int max_radius =
          4; // Max canopy size is 2.0, so overlap distance is max 4.0
      for (int dy = -max_radius; dy <= max_radius; dy++) {
        for (int dx = -max_radius; dx <= max_radius; dx++) {
          ...
          int nx = obj.x + dx;
          int ny = obj.y + dy;
          if (!map->is_in_bounds(nx, ny, obj.z))
            continue;

          for (ObjectUID other_uid : spatial_grid_.get_at(nx, ny, obj.z)) {
            ObjectInstance *other = get(other_uid);
            if (!other || !other->vegetation)
              continue;

            float dist_sq = (float)(dx * dx + dy * dy);
            float overlap_dist =
                obj.vegetation->canopy + other->vegetation->canopy;

            if (dist_sq < overlap_dist * overlap_dist) {
              if (other->vegetation->canopy_density >
                      obj.vegetation->canopy_density ||
                  other->vegetation->canopy > obj.vegetation->canopy) {
                float dist = std::sqrt(dist_sq);
                float overlap_amount = overlap_dist - dist;
                float shade_from_tree = std::min(
                    1.5f, overlap_amount * other->vegetation->canopy_density);
  ...
  // The tree sits at (x, y, z). The soil voxel is directly below at (x, y,
  // z-1).
  int soil_x = obj.x;
  int soil_y = obj.y;
  int soil_z = obj.z - 1;
  ...
  if (!is_world_gen) {
    ...
    if (soil_water > wilting) {
      ...
      obj.vegetation->moisture =
          std::min(1.0f, obj.vegetation->moisture + uptake * 5.0f);
    } else {
      obj.vegetation->moisture =
          std::max(0.0f, obj.vegetation->moisture - 0.03f);
    }
  }

  // Apply growth and health based on current moisture (which may have been
  // updated gradually by substeps)
...
void ObjectManager::drink_water_all(Game_map *map, float drink_amount,
                                    float dry_amount) {
  if (!map)
    return;
  for (auto &slot : slots_) {
    ...
    if (obj.vegetation && obj.health > 0.0f) {
      ...
      if (map->is_in_bounds(soil_x, soil_y, soil_z)) {
        ...
        if (t.state.liquid_volume > wilting) {
          float uptake =
              std::min(drink_amount, t.state.liquid_volume - wilting);
          t.state.liquid_volume -= uptake;
          float gain =
              (uptake / 0.02f) * 0.1f; // Matches the + uptake * 5.0f math
          obj.vegetation->moisture =
              std::min(1.0f, obj.vegetation->moisture + gain);
        } else {
          obj.vegetation->moisture =
              std::max(0.0f, obj.vegetation->moisture - dry_amount);
        }
      }
    }
  }
