...
void AnatomySystem::aggregateStatus(AnatomyComponent *anatomy,
                                    float &totalPain) {
...
float AnatomySystem::calculateReach(Entity *entity) {
  ...
  for (const auto &part : anatomy->body_parts) {
    if (part.type == BodyPartType::LIMB && part.limb_type == LimbType::ARM &&
        part.is_functional) {
...
float AnatomySystem::calculateMovementFactor(Entity *entity) {
  ...
  if (totalLegs == 0)
    return 1.0f; // No legs defined, assume flying or snake? :P

  if (functionalLegs == 0)
    return 0.0f; // Cant move
  if (functionalLegs < totalLegs)
    return 0.5f; // Hobble

  return 1.0f;
}

bool AnatomySystem::inflictWound(Entity *target, const std::string &partName,
...
static BodyPart *checkHitRecursive(AnatomyComponent &anatomy, int partIndex,
                                   float localX, float localY) {
  if (partIndex < 0 || partIndex >= (int)anatomy.body_parts.size())
    return nullptr;
  ...
  for (int childIdx : part.children_indices) {
    if (childIdx < 0 || childIdx >= (int)anatomy.body_parts.size())
      continue;
    ...
    BodyPart *hitChild =
        checkHitRecursive(anatomy, childIdx, childLocalX, childLocalY);
    if (hitChild) {
      return hitChild;
    }
  }
...
BodyPart *AnatomySystem::determineHitLocation(AnatomyComponent *anatomy,
                                              float localX, float localY) {
  if (!anatomy)
    return nullptr;
