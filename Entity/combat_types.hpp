...
struct DamageInfo {
  ...
  DamageInfo(float amt = 0.0f, DamageType t = DamageType::BLUNT,
             float pen = 0.0f, float crit = 2.0f,
             AttackType at = AttackType::MELEE, float rch = 0.0f,
             float lev = 1.0f)
      : amount(amt), type(t), armor_penetration(pen), critical_multiplier(crit),
        attack_type(at), reach(rch), leverage(lev) {}
};
...
struct AttackResult {
  ...
  AttackResult()
      : hit(false), critical(false), damage_dealt(0.0f), body_part_name(""),
        part_destroyed(false), arterial_hit(false), pain_inflicted(0.0f) {}
};
