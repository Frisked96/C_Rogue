...
public:
   ...
   */
  AttackResult resolveAttack(Entity *attacker, Entity *defender,
...
private:
  ...
  /**
   * @brief Recursively selects a target body part based on size and
   * accessibility.
   */
  ...
  /**
   * @brief Applies specific anatomical effects based on damage type (bleeding,
   * pain, etc).
   */
  void applySecondaryEffects(BodyPart *part, const DamageInfo &info,
                             float damageDealt, AttackResult &result);

  /**
   * @brief Distributes excess damage to parent parts if the target part is
   * destroyed.
   */
