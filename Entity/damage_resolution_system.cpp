...
AttackResult DamageResolutionSystem::resolveAttack(Entity *attacker,
                                                   Entity *defender,
  ...
  auto *defCombat =
      defender
          ->getComponent<CombatComponent>(); // To get defender's reach/defense
  ...
  if (defCombat) {
    ...
    defDefense = static_cast<float>(
        defCombat->defense); // Using defense as dodge/parry score
  }

  float reachDelta = info.reach - defReach;
  float reachBonus =
      reachDelta * 0.15f; // +15% hit chance per 1.0 reach advantage

  // Leverage vs Defense (Leverage helps break through guards)
  // If leverage > 1.0, it reduces the effective defense of the target for the
  // hit calculation
  ...
  float hitChance =
      (baseHitChance * accuracy) + reachBonus - (effectiveDefense * 0.05f);
  ...
  result.damage_dealt =
      static_cast<float>(previousHp - targetPart->current_hitpoints);
  ...
  if (finalDamage > result.damage_dealt) {
    handleDamageOverflow(defender, targetPart,
                         finalDamage - result.damage_dealt);
  }

  return result;
}

BodyPart *DamageResolutionSystem::selectTargetPart(AnatomyComponent *anatomy,
                                                   DamageType type) {
  if (anatomy->body_parts.empty())
    return nullptr;
  ...
  // Or consider all parts? Let's consider all parts for simplicity first,
  // weighted by size. However, internal organs shouldn't be hit directly unless
  // penetration happens. So filter for parts with no parent (root) or parts
  // that are explicitly "external". For now, let's assume all parts are
  // candidates but weight them. Actually, hitting the Heart directly without
  // hitting Chest is wrong. So we should select from Root parts, then drill
  // down.
  ...
  while (!selected->children_indices.empty() && roll(rng) < internalChance) {
    ...
    std::discrete_distribution<int> iDist(internalWeights.begin(),
                                          internalWeights.end());
...
float DamageResolutionSystem::calculateArmorReduction(BodyPart *part,
                                                      const DamageInfo &info) {
  float effectiveArmor =
      static_cast<float>(part->armor_value) * (1.0f - info.armor_penetration);
...
void DamageResolutionSystem::applySecondaryEffects(BodyPart *part,
                                                   const DamageInfo &info,
                                                   float damageDealt,
                                                   AttackResult &result) {
  // 1. Pain calculation
  ...
  if (part->pain_level > 100.0f)
    part->pain_level = 100.0f;
  result.pain_inflicted = part->pain_level;

  // 2. Bleeding
  float bleedingChance = 0.0f;
  if (info.type == DamageType::SHARP)
    bleedingChance = 0.8f;
  else if (info.type == DamageType::PIERCING)
    bleedingChance = 0.4f;
  else if (info.type == DamageType::BLUNT)
    bleedingChance = 0.1f;
  ...
  if (info.type == DamageType::SHARP || info.type == DamageType::PIERCING) {
    ...
    if (roll(rng) < arterialHitChance) {
      ...
      if (part->arterial_integrity < 0.0f)
        part->arterial_integrity = 0.0f;
      result.arterial_hit = true;
    }
  }
}

void DamageResolutionSystem::handleDamageOverflow(Entity *entity,
                                                  BodyPart *part,
                                                  float excessDamage) {
  // In a flat system, we can look up parent index directly
  if (part->parent_index != -1) {
    // Apply half of excess damage to parent?
    // We'd need access to anatomy component here, but we only have part* and
    // entity*. Entity* has anatomy.
