...
EntityID EntityManager::spawn(EntityType type, int x, int y, int z) {
  ...
  e.state.thirst_rate =
      1.0f /
      (p.daily_water_need > 0 ? (86400.0f / p.daily_water_need) : 86400.0f);
...
void EntityManager::kill(EntityID id) {
  ...
  if (index < slots.size() && slots[index].generation == get_generation(id)) {
    if (slots[index].active) {
      spatial_grid.remove(id, slots[index].entity.state.x,
                          slots[index].entity.state.y,
                          slots[index].entity.state.z);
...
void EntityManager::update(float dt) {
  for (auto &slot : slots) {
    ...
    if (e.state.stomach_contents > 0) {
      float digested =
          std::min(e.state.stomach_contents, e.state.digestion_rate * dt);
    ...
    // Clamping
    e.state.energy_stored =
        std::clamp(e.state.energy_stored, 0.0f, p.max_calories_stored);
    e.state.thirst = std::clamp(e.state.thirst, 0.0f, 1.0f);
    e.state.hunger = std::clamp(e.state.hunger, 0.0f, 1.0f);
    e.state.excretion_buffer =
        std::clamp(e.state.excretion_buffer, 0.0f, p.max_excretion_buffer);
    e.state.stomach_contents =
        std::clamp(e.state.stomach_contents, 0.0f, p.max_stomach);
  }
}
