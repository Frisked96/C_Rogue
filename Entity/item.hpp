...
enum class ItemType { WEAPON, ARMOR, CONSUMABLE, REAGENT, TOOL, MISC };
...
public:
  ...
  float
      leverage_bonus;  // Multiplier for accuracy or strength-based interactions
  float speed_penalty; // Larger weapons might be slower to swing

  Weapon(const std::string &name, float dmg,
         DamageType type = DamageType::SHARP, float rch = 1.0f, int weight = 5)
...
public:
  ...
  Armor(const std::string &name, int def, const std::string &slot = "torso",
        int weight = 10)
...
public:
  ...
  Consumable(const std::string &name, int potency, int charges = 1,
             int weight = 1)
      : Item(name, ItemType::CONSUMABLE, weight), potency(potency),
        charges(charges) {}
